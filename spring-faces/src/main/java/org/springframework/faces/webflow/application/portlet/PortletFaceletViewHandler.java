...
public class PortletFaceletViewHandler extends PortletViewHandler {
	...
	public PortletFaceletViewHandler(ViewHandler wrapped) {
		super(wrapped);
		logger.warn("*****");
		logger.warn("***** PLEASE UPDATE YOUR faces-confix.xml");
		logger.warn("*****");
		logger.warn("***** org.springframework.faces.webflow.application.portlet.PortletFaceletViewHandler has been deprecated");
		logger.warn("***** please update references to use org.springframework.faces.webflow.context.portlet.PortletViewHandler");
		logger.warn("*****");
	}
}
